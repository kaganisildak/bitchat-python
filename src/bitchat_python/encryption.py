...
class NoiseHandshakeState:
    ...
    def __init__(
        self,
        role: str,
        local_static_key: X25519PrivateKey,
        remote_static_key: Optional[X25519PublicKey] = None,
    ):
        self.role = role
    ...
    def write_message(self, payload: bytes = b"") -> bytes:
        ...
        for pattern in patterns:
            if pattern == "e":
                ...
                shared = self._dh(
                    self.local_ephemeral_private, self.remote_ephemeral_public
                )
                ...
                if self.role == NoiseRole.INITIATOR:
                    if (
                        ...
                        or not self.remote_static_public
                    ):
                        raise NoiseError("Missing keys for es")
                    shared = self._dh(
                        self.local_ephemeral_private, self.remote_static_public
                    )
                else:
                    if (
                        not self.local_static_private
                        or not self.remote_ephemeral_public
                    ):
                        raise NoiseError("Missing keys for es")
                    shared = self._dh(
                        self.local_static_private, self.remote_ephemeral_public
                    )
                self._mix_key(shared)
    ...
    def read_message(self, message: bytes) -> bytes:
        ...
        for pattern in patterns:
            if pattern == "e":
                ...
                if len(buffer) < 32:
                    raise NoiseError(
                        "Invalid message: insufficient data for ephemeral key"
                    )
                ephemeral_data = buffer[:32]
                buffer = buffer[32:]

                self.remote_ephemeral_public = X25519PublicKey.from_public_bytes(
                    ephemeral_data
                )
                ...
                key_length = (
                    48 if self.cipher_state.has_key() else 32
                )  # 32 + 16 tag if encrypted
                if len(buffer) < key_length:
                    raise NoiseError(
                        "Invalid message: insufficient data for static key"
                    )
...
class EncryptionService:
    ...
    def _load_or_create_identity(
        self, identity_path: Optional[str]
    ) -> X25519PrivateKey:
    ...
    def process_handshake_message(
        ...
        if peer_id in self.handshake_states:
            ...
            handshake = NoiseHandshakeState(
                NoiseRole.RESPONDER, self.static_identity_key
            )
            self.handshake_states[peer_id] = handshake
