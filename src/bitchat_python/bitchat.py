...
class BitchatClient:
        ...
        while not device and self.running:
            ...
            if not device:
                ...
                if scan_attempts == max_initial_attempts:
                    ...
                    print(
                        "\033[90m» You can start using commands while waiting.\033[0m"
                    )
        ...
        if should_fragment(packet):
            ...
            try:
                await self.client.write_gatt_char(
                    self.characteristic, packet, response=write_with_response
                )
            except Exception as e:
    ...
    async def handle_announce(self, packet: BitchatPacket):
        ...
        if is_new_peer:
            ...
            logger.info(
                f"[CRYPTO] Sending key exchange to new peer {packet.sender_id_str}"
            )
            key_exchange_payload = (
                self.encryption_service.get_combined_public_key_data()
            )
        ...
        if (
            ...
            try:
                decrypted = self.encryption_service.decrypt_with_key(
                    message.encrypted_content, self.channel_keys[message.channel]
                )
    ...
    async def handle_key_exchange(self, packet: BitchatPacket):
        ...
        if len(packet.payload) >= 96:
            try:
                self.encryption_service.store_peer_keys(
                    packet.sender_id_str, packet.payload
                )
                logger.info(
                    f"[CRYPTO] Key exchange completed with {packet.sender_id_str}"
                )
                ...
                logger.info(
                    f"[CRYPTO] Key exchange failed with {packet.sender_id_str}: {e}"
                )
        else:
            logger.info(
                f"[CRYPTO] Invalid key exchange payload size: {len(packet.payload)}"
            )
        ...
        if payload_str.startswith("#"):
            ...
            if disconnected_peer and disconnected_peer.nickname:
                ...
                if (
                    ...
                    self.chat_context.switch_to_public()
                    print(
                        "\033[90m» Switched to public chat (peer disconnected)\033[0m\n> ",
                        end="",
                        flush=True,
                    )
            ...
            if len(self.peers) == 0:
                print(
                    "\033[90m» You're now the only one in the network.\033[0m\n> ",
                    end="",
                    flush=True,
                )
    ...
    async def handle_delivery_ack(self, packet: BitchatPacket, raw_data: bytes):
        ...
        if is_for_us:
            ...
            if packet.ttl == 3 and self.encryption_service.has_peer_key(
                packet.sender_id_str
            ):
                try:
                    ack_payload = self.encryption_service.decrypt(
                        packet.payload, packet.sender_id_str
                    )
                except (
                    Exception
            ...
            try:
                ...
                if self.delivery_tracker.mark_delivered(ack.original_message_id):
                    print(
                        f"\r\033[K\033[90m✓ Delivered to {ack.recipient_nickname}\033[0m\n> ",
                        end="",
                        flush=True,
                    )
    ...
    async def send_channel_announce(
        self, channel: str, is_protected: bool, key_commitment: Optional[str]
    ):
        ...
        packet = create_bitchat_packet(
            self.my_peer_id, MessageType.CHANNEL_ANNOUNCE, payload.encode()
        )
    ...
    async def handle_user_input(self, line: str):
        ...
        if line == "/channels":
            all_channels = set(self.chat_context.active_channels) | set(
        ...
        if line == "/status":
            ...
            connection_status = (
                "Connected" if (self.client and self.client.is_connected) else "Offline"
            )
        ...
        if self.chat_context.current_chat.mode is ChatMode.PrivateDM:
            ...
            if not self.client or not self.client.is_connected:
                ...
                print(
                    "\033[90mYour message will be sent once someone joins the network.\033[0m"
                )
                print(
                    "\033[90m(This Python client doesn't queue messages while offline)\033[0m"
                )
            else:
    ...
    async def handle_join_channel(self, line: str):
        ...
        if not all(c.isalnum() or c in "-_" for c in channel_name[1:]):
            ...
            print(
                "\033[90mChannel names can only contain letters, numbers, hyphens and underscores.\033[0m"
            )
            return
        ...
        if channel_name in self.password_protected_channels:
            ...
            if not password:
                print(
                    f"❌ Channel {channel_name} is password-protected. Use: /j {channel_name} <password>"
                )
                return
            ...
            if channel_name in self.channel_key_commitments:
                ...
                if test_commitment != self.channel_key_commitments[channel_name]:
                    print(
                        f"❌ wrong password for channel {channel_name}. please enter the correct password."
                    )
                    return
            ...
            print(
                f"\033[90m» Joined password-protected channel: {channel_name} 🔒\033[0m"
            )
            ...
            if password:
                ...
                print("\r\033[K\033[90m─────────────────────────\033[0m")
                print(
                    f"\033[90m» Joined password-protected channel: {channel_name} 🔒. Just type to send messages.\033[0m"
                )
    ...
    async def handle_dm_command(self, line: str):
        ...
        if not self.client or not self.client.is_connected:
            ...
            print(
                "\033[90mWait for a connection before sending direct messages.\033[0m"
            )
            return
        ...
        if len(parts) == 1:
            ...
            if self.blocked_peers:
                ...
                for peer_id, peer in self.peers.items():
                    ...
                    if (
                        fingerprint
                        and fingerprint in self.blocked_peers
                        and peer.nickname
                    ):
                ...
                if blocked_nicks:
                    ...
                    print(
                        f"» Blocked peers (not currently online): {len(self.blocked_peers)}"
                    )
            else:
            ...
            if target_peer_id:
                ...
                if fingerprint:
                    if fingerprint in self.blocked_peers:
                        ...
                        print(
                            f"\033[90m{target} will no longer be able to send you messages.\033[0m"
                        )
                else:
                ...
                print(
                    "\033[90mThey may be offline or haven't sent any messages yet.\033[0m"
                )
        else:
    ...
    async def handle_pass_command(self, line: str):
        ...
        if old_key:
            ...
            try:
                ...
                notify_packet = create_bitchat_packet(
        ...
        init_packet = create_bitchat_packet(
            self.my_peer_id, MessageType.MESSAGE, init_payload
        )
    ...
    async def handle_transfer_command(self, line: str):
        ...
        if not new_owner_id:
            print(f"\033[93m⚠ User '{target}' not found\033[0m")
            print(
                "\033[90mMake sure they are online and you have the correct nickname.\033[0m"
            )
            return
    ...
    async def send_public_message(self, content: str):
        ...
        if not self.client or not self.characteristic:
            ...
            print(
                "\033[90mYour message will be sent once a connection is established.\033[0m"
            )
            return
        ...
        if self.chat_context.current_chat is ChatMode.Channel:
            ...
            if (
                ...
                print(
                    f"❌ Cannot send to password-protected channel {current_channel}. Join with password first."
                )
                return
    ...
    async def send_private_message(
        ...
        target_size = next(
            (size for size in block_sizes if len(payload) + 16 <= size), len(payload)
        )
        ...
        if 0 < padding_needed <= 255:
            ...
            logger.info(f"[PRIVATE] Added {padding_needed} bytes of PKCS#7 padding")

        # Encrypt
        try:
            encrypted = self.encryption_service.encrypt(
                bytes(padded_payload), target_peer_id
            )
            logger.info(f"[PRIVATE] Encrypted payload: {len(encrypted)} bytes")
            ...
            print(
                f"[!] Make sure you have received key exchange from {target_nickname}"
            )
...
def parse_bitchat_packet(data: bytes) -> BitchatPacket:
    ...
    # has_signature = (flags & FLAG_HAS_SIGNATURE) != 0  # FIXME: variable never used
...
def create_bitchat_packet_with_recipient_and_signature(
    ...
    return create_bitchat_packet_with_recipient(
        sender_id, recipient_id, msg_type, payload, signature
    )
...
def create_encrypted_channel_message_payload(
    ...
    # Encrypted content
    try:
        encrypted_content = encryption_service.encrypt_with_key(
            content.encode(), channel_key
        )
    except Exception as e:
        print(f"[!] Failed to encrypt message: {e}")
        return create_bitchat_message_payload_full(
            sender, content, channel, False, sender_peer_id
        )
