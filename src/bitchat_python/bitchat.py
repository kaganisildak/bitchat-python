...
class BitchatClient:
    def __init__(self):
        ...
        self.pending_private_messages: Dict[
    ...
    async def handle_noise_handshake_init(self, packet: BitchatPacket):
        ...
        try:
            ...
            if self.encryption_service.is_session_established(packet.sender_id_str):
                ...
                print(
                    f"\r\033[K\033[92m✓ Secure session established with {peer_nickname}\033[0m"
                )
                print("> ", end="", flush=True)
                ...
                if len(decrypted_payload) > 0 and decrypted_payload[0] == 1:
                    ...
                    if inner_packet:
                        ...
                        if inner_packet.msg_type == MessageType.MESSAGE:
                            ...
                            try:
                                message = parse_bitchat_message_payload(
    ...
    async def handle_delivery_ack(self, packet: BitchatPacket, raw_data: bytes):
        ...
        if is_for_us:
            ...
            if packet.ttl == 3 and self.encryption_service.is_session_established(
                packet.sender_id_str
            ):
                try:
    ...
    async def handle_user_input(self, line: str):
        ...
        if line == "/status":
            ...
            pending_messages = sum(
                len(msgs) for msgs in self.pending_private_messages.values()
            )
            ...
            if session_count > 0:
                ...
                for peer_id in self.encryption_service.get_active_peers():
                    nickname = (
                        self.peers.get(peer_id, Peer()).nickname or peer_id[:8] + "..."
                    )
                    fingerprint = self.encryption_service.get_peer_fingerprint(peer_id)
                    print(
                        f"  • {nickname} ({fingerprint[:8] if fingerprint else 'Unknown'}...)"
                    )
        ...
        if not self.encryption_service.is_session_established(target_peer_id):
            ...
            if target_peer_id in self.handshake_attempt_times:
                ...
                if current_time - last_attempt < self.handshake_timeout:
                    ...
                    print(
                        f"\033[90m» Handshake already in progress with {target_nickname}, please wait...\033[0m"
                    )
                    return
            ...
            try:
                ...
                print(
                    f"\033[91m✗ Failed to initiate secure connection with {target_nickname}\033[0m"
                )
                return

            print(
                f"\033[90m» Initiating secure handshake with {target_nickname}...\033[0m"
            )
            print(
                f"\033[90m» Your message will be sent automatically once the handshake completes.\033[0m"
            )
            return
        ...
        try:
            ...
            try:
                ...
                if "could not complete without blocking" in str(send_error):
                    ...
                    try:
                        print(
                            f"\033[90m» Message queued (BLE congestion), retrying...\033[0m"
                        )
                    except BlockingIOError:
                    ...
                    try:
                        ...
                        if self.characteristic:
                            ...
                            try:
                                ...
                                signature = self.encryption_service.sign_data(
                                    binding_data
                                )

                                # Encode to binary format
                                identity_payload = (
                                    self.encode_noise_identity_announcement_binary(
                                        self.my_peer_id,
                                        public_key_bytes,
                                        signing_public_key_bytes,
                                        self.nickname,
